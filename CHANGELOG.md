...
- Post-deploy smoke checks covering `/api/status` and `/api/stripe/products`, gated Playwright E2E job, and `docs/ACCEPTANCE.md`/`docs/VERIFY.md` guidance.
...
- Run `RUN_OPEN_E2E=true npm run test:e2e` to cover the open Playwright suite, and `pnpm test:e2e -- tests/e2e/authenticated.spec.ts` once `TEST_SESSION_COOKIE` is provisioned.
