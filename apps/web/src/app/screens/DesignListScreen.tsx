      ...
      {runs.length === 0 ? (
        ...
        <View className="gap-4">
          {runs.map((run) => {
            ...
            const deleteControl = Platform.OS === 'web' ? (
              <button
                type="button"
                onClick={(event) => {
            ...
            const content = (
              <>
                <CardHeader>
                  <View className="flex flex-row items-start justify-between gap-4">
                    <View className="flex-1">
                      ...
                      <CardDescription>
                        {run.config?.prompt
                          ? run.config.prompt.slice(0, 120) + (run.config.prompt.length > 120 ? '...' : '')
                          : 'No prompt provided'}
                      </CardDescription>
                    </View>
                ...
                <CardContent className="flex flex-col gap-3 border-t border-slate-100 pt-4">
                  ...
                  <View className="relative z-20 flex flex-row items-center justify-between gap-2">
                    <Button
                      variant="secondary"
                      size="sm"
            ...
            const wrapper = Platform.OS === 'web' ? (
              <View className={cn('relative z-0', isDeleting && 'opacity-60')}>
                <a
                  href={href}
                ...
                />
                <View className="relative">{content}</View>
              </View>
            ) : (
              <Pressable
                onPress={() => onViewRun(run.id)}
