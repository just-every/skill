...
This project ships a Cloudflare Worker that fronts authentication (Stytch), billing (Stripe), data (D1), and assets (R2). The `bootstrap.sh` script automates most resource provisioning so `wrangler deploy` can claim the routes immediately.
...
7. Pushes Worker secrets (`STYTCH_SECRET`, `STRIPE_SECRET_KEY`, `STRIPE_WEBHOOK_SECRET`) via `wrangler secret put` unless `SYNC_SECRETS=0`.
...
- **Stytch bearer rejected** – Confirm the frontend is sending a current `session_jwt` (via the React SDK) and that the Worker has the latest `STYTCH_SECRET`.
