...
3. **Billing persistence & Stripe checkout**
   - Billing flows now ride the hosted profile popup; `/app/billing` opens the popup Billing section rather than local UI. Worker helper/type fixes in `workers/api/src/index.ts` and `workers/api/test/helpers.ts` still ensure the worker persists billing emails, normalizes Stripe product metadata, and checkout/portal mocks validate behavior. Typecheck `pnpm --filter @justevery/worker run typecheck` and worker tests (`npm run test --workspace workers/api`) cover these paths.  
